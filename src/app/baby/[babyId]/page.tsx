...
const CATS = [
  {
    key: 'meals', label: 'Meals', sub: 'Track feeding',
  ...
  {
    key: 'wellbeing', label: 'Well-being', sub: 'Health & milestones',
  ...
  {
    key: 'stats', label: 'Stats', sub: 'Weight & growth',
  ...
  {
    key: 'caregivers', label: 'Caregivers', sub: 'Manage access',
...
export default function BabyPage() {
  ...
  if (!baby) return <div className="page-bg flex items-center justify-center min-h-screen"><p style={{ color: 'var(--text3)' }}>Loading…</p></div>

  const ageMonths = getAgeInMonths(baby.dob)
  const ageLabel = ageMonths < 24 ? `${ageMonths} mo` : `${Math.floor(ageMonths / 12)} yr`
  ...
  return (
    <div className="page-bg flex flex-col min-h-screen">
      <Topbar
        ...
        backLabel="Home"
        backHref="/dashboard"
        action={{ label: 'Profile', onClick: () => router.push(`/baby/${babyId}/profile`) }}
      />
      ...
      <div className="scroll-body">
        ...
        <div className="grid grid-cols-3 gap-2 mb-5">
          {[
            { label: 'Age', value: ageLabel, bg: '--rose-bg' },
            { label: 'Weight', value: latestWeight ? `${latestWeight.value} kg` : '—', bg: '--mint-bg' },
            { label: 'Height', value: latestHeight ? `${latestHeight.value} cm` : '—', bg: '--blue-bg' },
          ].map((s) => (
        ...
        {recentMeals.length > 0 && (
          <>
            <div className="sec-title">Recent meals</div>
            {recentMeals.map((m) => (
